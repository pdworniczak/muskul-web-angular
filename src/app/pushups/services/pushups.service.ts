...
export class PushupsService {
  ...
  getAllPushupsTraining(token: string): Observable<Array<Training | Test>> {
    return of(mockData[token].trainings)
      .pipe(delay(1000))
  }

  getLatsPushupsTraining(token: string): Observable<Training | Test> {
    const latestTraing = mockData[token].trainings.reduce((latestTraining, training) => {
      if (latestTraining) {
    ...
    return of(latestTraing)
      .pipe(delay(1000))
  }

  getCurrentPushupTrainingPlan(token: string): Observable<Plan> {
    return this.getLatsPushupsTraining(token)
      .pipe(map((training) => {
  ...
  _findScopePlan(score: number): [Scope, { [day: number]: Serie3Day | Serie5Day}] {
    const [scope, trainingWeek] = Object.entries(pushupsPlan).find(([scope, trainingWeek]) => {
